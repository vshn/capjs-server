...
def test_solve_produces_valid_solutions():
    cap = CapServer(secret_key="test", challenge_count=2, challenge_size=8, challenge_difficulty=1)
...
def test_solve_returns_list_of_ints():
    cap = CapServer(secret_key="test", challenge_count=3, challenge_size=8, challenge_difficulty=1)
    data = cap.create_challenge()
    solutions = solve(data["token"], data["challenge"])
